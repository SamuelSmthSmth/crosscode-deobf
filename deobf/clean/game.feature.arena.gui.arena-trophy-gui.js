ig.module("game.feature.arena.gui.arena-trophy-gui").requires("impact.feature.gui.gui", "impact.feature.gui.base.basic-gui", "impact.feature.gui.base.box", "game.feature.gui.base.text", "game.feature.menu.gui.menu-misc", "game.feature.menu.gui.arena.arena-misc", "game.feature.arena.gui.arena-effect-display", "game.feature.gui.widget.modal-dialog").defines(function() {
    sc.ArenaCupOverview = ig.GuiElementBase.extend({
        ...
        init: function(cup, callback) {
            ...
            var noRush = sc.arena.getCupCore(this.cup).noRush || false;
            this.content = new ig.GuiElementBase;
            this.content.setSize(280, 166 - (noRush ? 40 : 0));
            this.header =
                new sc.TextGui("");
            this.header.setAlign(ig.GUI_ALIGN_X.CENTER, ig.GUI_ALIGN_Y.TOP);
            this.header.setPos(0, -4);
            this.content.addChildGui(this.header);
            var gui = new ig.ColorGui("#7E7E7E", 280, 1);
            gui.setAlign(ig.GUI_ALIGN_X.CENTER, ig.GUI_ALIGN_Y.TOP);
            gui.setPos(0, 13);
            ...
            gui = new ig.ColorGui("#7E7E7E", 280, 1);
            gui.setAlign(ig.GUI_ALIGN_X.CENTER, ig.GUI_ALIGN_Y.TOP);
            gui.setPos(0, 13 + this.list.hook.size.y - 1);
            this.content.addChildGui(gui);
            ...
            this.rushTime.setPos(-3, this.time.hook.pos.y + 20);
            this.rushTime.setTime(0, true);
            !noRush && this.content.addChildGui(this.rushTime);
            this.rushTime.hook.pos.x = this.rushTime.hook.pos.x + 24;
            gui = new ig.ImageGui(this.gfx, 512, 207, 17, 16);
            gui.setPos(-17, -1);
            this.rushTime.addChildGui(gui);
            ...
            this.rushChain.setPos(-3, this.rushTime.hook.pos.y + 20);
            this.rushChain.setValueAsNumber(0, true);
            !noRush && this.content.addChildGui(this.rushChain);
            ...
            if (this.trophyType > 0) {
                ...
                noRush = ig.lang.get("sc.gui.arena.medals")[this.trophyType - 1];
                this.trophyFooter = new sc.TextGui("\\c[4]" + noRush + "\\c[0]", {
                    font: sc.fontsystem.smallFont,
                    maxWidth: 76,
                    textAlign: ig.Font.ALIGN.RIGHT,
                    linePadding: -3
                });
                this.trophyFooter.setAlign(ig.GUI_ALIGN_X.RIGHT,
                ...
                this.trophyHeader =
                    new sc.TextGui("\\c[4]" + ig.lang.get("sc.gui.arena.overview.trophyNone") + "\\c[9]", {
                        font: sc.fontsystem.smallFont,
                        maxWidth: 80,
            ...
            this.msgBox.setAlign(ig.GUI_ALIGN.X_CENTER,
            ...
            if (this.blockTimer > 0) this.blockTimer = this.blockTimer - ig.system.tick;
                ...
                if (this.addEntries) {
                    ...
                    if (this.timer <= 0)
                        if (this.currentIndex == -1) {
                            ...
                            var entry = this.entries[this.currentIndex],
                                medalEntry = new sc.ArenaCupOverview.MedalEntry(entry.id, entry.medal, false, entry.isRush),
                                row = ~~(entry.id / 10);
                            medalEntry.setPos(1 + entry.id % 10 * 18, 2 + row * 18);
                            this.medals.addChildGui(medalEntry);
                            this.totalValue = this.totalValue + entry.points;
                            this.total.setValueAsNumber(this.totalValue);
                            if (entry.time) {
                                this.totalTime = this.totalTime + entry.time;
                                this.time.setTime(this.totalTime)
                            }
                            this.currentIndex++;
                            this.medals.hook.size.y = 2 + (row + 1) * 18;
                            ...
                            if (this.currentIndex >= this.entries.length) {
                                ...
                                this.trophyFooter && this.trophyFooter.doStateTransition("DEFAULT", false, false, null, 0.4)
                            }
                        }
                    ...
                    if (this.timer <= 0) {
                        ...
                        this.totalOverlay.doStateTransition("DEFAULT",
                            false, false,
                            function() {
        ...
        show: function(headerText) {
            ...
            this.blockTimer = 0.1;
            headerText = headerText || ig.lang.get("sc.gui.arena.overview.cupHeader").replace("[CUP_NAME]", sc.arena.getCupName(this.cup));
        ...
        addMedals: function() {
            ...
            this.time.setTime(0, true);
            this.rushTime.setTime(0, true);
            this.rushChain.setValueAsNumber(0,
                true);
            this.total.setValueAsNumber(0, true);
            var progress = sc.arena.getCupProgress(this.cup),
                noRush = sc.arena.getCupCore(this.cup).noRush || false;
            if (progress) {
                noRush || this.entries.push({
                    id: 0,
                    medal: progress.rush.medal || 0,
                    points: progress.rush.points || 0,
                    isRush: true
                });
                for (var rounds = progress.rounds, i = 0; i < rounds.length; i++) {
                    var round = rounds[i];
                    this.entries.push({
                        id: noRush ? i : i + 1,
                        medal: round.medal || 0,
        ...
        skip: function() {
            if (!this.done) {
                ...
                for (var i = this.currentIndex; i < this.entries.length; i++) {
                    var entry = this.entries[this.currentIndex],
                        medalEntry = new sc.ArenaCupOverview.MedalEntry(entry.id, entry.medal, true, entry.isRush),
                        row = ~~(entry.id / 10);
                    medalEntry.setPos(1 + entry.id % 10 * 18, 2 + row * 18);
                    this.medals.addChildGui(medalEntry);
                    this.totalValue = this.totalValue + entry.points;
                    if (entry.time) this.totalTime = this.totalTime + entry.time;
                    this.currentIndex++;
                    this.medals.hook.size.y = 2 + (row + 1) * 18
                }
                this.total.setValueAsNumber(this.totalValue);
    ...
    sc.ArenaCupOverview.MedalEntry = ig.GuiElementBase.extend({
        ...
        overlay: null,
        rush: null,
        init: function(id, medal, instant, isRush) {
            this.parent();
            this.setSize(16, 16);
            this.medal = medal || 0;
            ...
            this.medalGui.doStateTransition("DEFAULT",
                instant);
            this.addChildGui(this.medalGui);
            if (medal > 0 && !instant) {
                this.overlay = new ig.ImageGui(this.gfx, this.medal * 16, 16, 16, 16);
            ...
            if (isRush) {
                id = new sc.TextGui(ig.lang.get("sc.gui.arena.rushAbbreviated"), {
                    font: sc.fontsystem.tinyFont
                });
                id.setAlign(ig.GUI_ALIGN_X.CENTER, ig.GUI_ALIGN_Y.CENTER);
                id.setPos(1, 1);
                this.rush = new ig.ColorGui("#000", id.hook.size.x + 1, id.hook.size.y - 1);
                this.rush.hook.localAlpha = 1;
                this.rush.setAlign(ig.GUI_ALIGN_X.LEFT, ig.GUI_ALIGN_Y.TOP);
                this.rush.addChildGui(id);
