 ...
 * Source module: `assets/js/game.compiled.js` → `ig.module("game.feature.menu.gui.lore.lore-menu")`.
 *
 * `sc.LoreMenu`: the lore (codex) submenu (`sc.BaseMenu`) — the lore
 * list with its info box, the sort menu (order / name / unlocked) and
 * the help hotkey.
 */
ig.module("game.feature.menu.gui.lore.lore-menu")
    .requires("impact.feature.gui.gui", "impact.feature.gui.base.basic-gui", "game.feature.menu.gui.base-menu", "game.feature.menu.gui.menu-misc", "game.feature.control.control", "game.feature.menu.gui.lore.lore-list")
    .defines(function () {
    ...
    sc.LoreMenu = sc.BaseMenu.extend({
        ...
        modelChanged: function (model, event, data) {
            if (model == sc.menu)
                ...
                this.info.setCategory(this.list.getCurrentTabKey());
                this.updateSortMenuButton(this.list.getCurrentSortText())
            } else event == sc.MENU_EVENT.SYNOP_FOCUS ? this.info.setFocus(data) : event == sc.MENU_EVENT.SYNOP_SET_INFO && this.info.setLore(sc.menu.synopInfo)
        }
    })
});
ig.baked = !0;
