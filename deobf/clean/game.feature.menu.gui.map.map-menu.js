 ...
 * Source module: `assets/js/game.compiled.js` → `ig.module("game.feature.menu.gui.map.map-menu")`.
 *
 * `sc.MapMenu`: the Map submenu container. Hosts the area map, the worldmap
 * overlay, the floor buttons, chest/stamp displays, the current-area label
 * and the stamp-edit popup, plus the three top-bar hotkeys (help, center map,
 * worldmap). Routes worldmap/area-load/stamp menu events.
 */
ig.module("game.feature.menu.gui.map.map-menu")
    .requires("impact.feature.gui.gui", "impact.feature.gui.base.basic-gui", "game.feature.menu.gui.map.map-misc", "game.feature.menu.gui.map.map-stamp", "game.feature.menu.gui.map.map-area", "game.feature.menu.gui.map.map-worldmap")
    .defines(function () {
    ...
    sc.MapMenu = sc.BaseMenu.extend({
        ...
        init: function () {
            ...
            this.hotkeyWorldmap = new sc.ButtonGui("\\i[help2]" + ig.lang.get("sc.gui.menu.hotkeys.worldmap"), void 0, true, sc.BUTTON_TYPE.SMALL);
        ...
        onCenterButtonPressed: function (addStamp) {
            if (addStamp) {
                this.area.addStamp()
            } else {
                sc.BUTTON_SOUND.submit.play();
                var message = ig.lang.get("sc.gui.menu.map-menu.popupTitle") + "\n\n" + ig.lang.get("sc.gui.menu.map-menu.popupDesc");
        ...
        onWorldmapButtonPressed: function () {
            if (sc.menu.mapWorldmapActive) {
                ...
                sc.menu.enterWorldMap()
            }
        },

        createHelpGui: function () {
            if (!this.helpGui) {
                this.helpGui = new sc.HelpScreen(this, ig.lang.get("sc.gui.menu.help-texts.map.title"), ig.lang.get("sc.gui.menu.help-texts.map.pages"), function () {
        ...
        onAddHotkeys: function (commitToTopBar) {
            ...
            sc.menu.buttonInteract.addGlobalButton(this.hotkeyWorldmap, this.onHotkeyWorldmapCheck.bind(this));
        ...
        modelChanged: function (menu, event, data) {
            if (menu == sc.menu) {
                if (event == sc.MENU_EVENT.MAP_WORLDMAP_STATE) {
                    ...
                    this.chestDisplay.doStateTransition("DEFAULT");
                    this.stampDisplay.doStateTransition("DEFAULT")
                } else if (event == sc.MENU_EVENT.MAP_OPEN_STAMPS) {
                    this.stamps.hide(true);
                    this.stamps.show(data)
                }
            }
        }
    })
