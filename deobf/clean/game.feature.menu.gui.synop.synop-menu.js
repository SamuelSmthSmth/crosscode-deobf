 ...
 * @description The Synopsis (start screen) menu (sc.SynopsisMenu): the column
 *   of submenu buttons (trade, botanics, enemies, map, lore, trophies, records,
 *   newgame) with the current task and activity log displays.
 */
ig.module("game.feature.menu.gui.synop.synop-menu").requires("impact.feature.gui.gui", "impact.feature.gui.base.basic-gui", "game.feature.menu.gui.help-boxes", "game.feature.menu.gui.synop.synop-misc").defines(function() {
	var buttonYs = [43, 73, 103, 133, 163, 193, 223, 253, 283];
	sc.SynopsisMenu = sc.BaseMenu.extend({
		...
		init: function() {
			...
			this.buttonGroup.addSelectionCallback(function(button) {
				button.data && (button.active ? sc.menu.setInfoText(button.data) : sc.menu.setInfoText("???"))
			});
			this.buttonGroup.setMouseFocusLostCallback(function() {
				sc.menu.setInfoText("", true)
			});
			var player = sc.model.player;
			this.buttons.trade = this._createButton("trade", 0, sc.MENU_SUBMENU.TRADE, !player.hasItem(135) || !sc.trade.hasAnyTraderFound());
			this.buttons.botanics = this._createButton("botanics", 1, sc.MENU_SUBMENU.BOTANICS, !player.hasItem(285) || !sc.menu.hasAnyDropFound());
			this.buttons.enemies = this._createButton("enemies", 2, sc.MENU_SUBMENU.ENEMY, !player.hasItem(135));
			this.buttons.map = this._createButton("map", 3, sc.MENU_SUBMENU.MAP);
			this.buttons.lore = this._createButton("lore", 4, sc.MENU_SUBMENU.LORE, !player.hasItem(135), true);
			this.buttons.trophies = this._createButton("trophies", 5, sc.MENU_SUBMENU.TROPHY, false, true);
			this.buttons.records = this._createButton("records", 6, sc.MENU_SUBMENU.STATS, !player.hasItem(135));
			...
			this.buttons.lore.setActive(player.hasItem(135));
			this.buttons.records.setActive(player.hasItem(135));
			this.buttons.trade.setActive(player.hasItem(135) && sc.trade.hasAnyTraderFound());
			this.buttons.enemies.setActive(player.hasItem(135));
			this.buttons.botanics.setActive(player.hasItem(285) && sc.menu.hasAnyDropFound());
			this.buttons.newgame && this.buttons.newgame.setActive(sc.newgame.active);
			for (var key in this.buttons) this.buttons[key] && this.addChildGui(this.buttons[key]);
			...
			this.infoTask = new sc.SynopsisTaskDisplay;
		...
		showMenu: function() {
			...
			this.buttons.records.doStateTransition("DEFAULT", false, false, null, 0.016);
		...
		exitMenu: function() {
			for (var key in this.buttons) {
				...
				button && button.doStateTransition("HIDDEN")
			}
			this.infoTask.hide();
			this.infoLogs.hide();
		...
		_createButton: function(key, index, menu, disabled, newUnlockButton) {
			var button = null,
				button = newUnlockButton ? new sc.NewUnlockButton(disabled ? "???" : ig.lang.get("sc.gui.menu.synopsis-menu." + key), sc.BUTTON_MENU_WIDTH) : new sc.ButtonGui(disabled ? "???" : ig.lang.get("sc.gui.menu.synopsis-menu." + key), sc.BUTTON_MENU_WIDTH);
			button.setAlign(ig.GUI_ALIGN.X_RIGHT, ig.GUI_ALIGN.Y_TOP);
			button.setPos(10, buttonYs[index] + 0);
			...
			button.setData(ig.lang.get("sc.gui.menu.synopsis-menu.descriptions." + key));
			button.menu = menu;
		...
		createHelpGui: function() {
			if (!this.helpGui) {
				this.helpGui = new sc.HelpScreen(this, ig.lang.get("sc.gui.menu.help-texts.synopsis.title"), ig.lang.get("sc.gui.menu.help-texts.synopsis.pages"), function() {
