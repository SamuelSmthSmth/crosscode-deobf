 ...
 * @description ig.GUI.SergeyMode: the easter-egg "Sergey Mode" fullscreen
 *   scrolling pattern overlay with corner art and start/end sounds.
 */
ig.module("game.feature.gui.widget.sergey-mode").requires("impact.feature.gui.gui").defines(function() {
	ig.GUI.SergeyMode = ig.GuiElementBase.extend({
		...
		updateDrawables: function(drawables) {
			drawables.addPattern(this.patternSheet.getPattern(0), 0, 0, this.scroll, this.scroll, ig.system.width, ig.system.height).setCompositionMode("lighter");
			drawables.addGfx(this.cornerImage, 0, 20, 0, 0, 64, 64, false, false).setCompositionMode("lighter");
			drawables.addGfx(this.cornerImage, ig.system.width - 64, 20, 0, 0, 64, 64, true, false).setCompositionMode("lighter");
			drawables.addGfx(this.cornerImage, ig.system.width - 64, ig.system.height - 64 - 20, 0, 0, 64, 64, true, true).setCompositionMode("lighter");
			drawables.addGfx(this.cornerImage, 0, ig.system.height - 64 - 20, 0, 0, 64, 64, false, true).setCompositionMode("lighter")
		},
		remove: function() {
			this.endSound.play();
