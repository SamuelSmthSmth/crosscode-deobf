ig.module("game.feature.quick-menu.gui.quick-menu").requires("impact.base.image", "impact.feature.gui.gui", "game.feature.quick-menu.gui.quick-screen", "game.feature.quick-menu.gui.circle-menu", "game.feature.quick-menu.gui.quick-item-menu", "game.feature.quick-menu.gui.quick-party", "game.feature.menu.gui.menu-misc").defines(function() {
    sc.QuickMenu = ig.GuiElementBase.extend({
        ...
        modelChanged: function(model, msg, data) {
            if (model == sc.model) {
                model.isCutscene() && this.location.forceHide();
            ...
            } else if (model == sc.quickmodel)
                if (msg == sc.QUICK_MODEL_EVENT.SWITCH_STATE) {
                    this._setInfoBarAndLocation();
                    this.buffStats.hide();
                    this.buffInfo.setText("")
                } else if (msg ==
                sc.QUICK_MODEL_EVENT.INFO_TEXT_CHANGED) this.info.setText(sc.quickmodel.infoText, data ? 0.5 : 0);
            else {
                if (msg == sc.QUICK_MODEL_EVENT.BUFF_TEXT_CHANGED) {
                    data = data || false;
                    this.buffInfo.setText(sc.quickmodel.buffText, data ? 0.5 : 0);
                    this.buffStats.show(sc.quickmodel.buffID, data ? 0.5 : 0)
                }
            } else model == sc.map ? msg == sc.MAP_EVENT.MAP_ENTERED && sc.options.get("quick-location") == sc.QUICK_LOCATION_OPTION.MAP && !sc.model.isCutscene() && sc.model.player.getCore(sc.PLAYER_CORE.QUICK_MENU) && (sc.arena.active || this.location.show(true)) : model == sc.message && msg ==
