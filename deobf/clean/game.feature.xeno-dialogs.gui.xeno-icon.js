 ...
 *
 * Floating dialog bubble shown above entities that can start a xeno dialog.
 * Displays the current line with typewriter text, a skip indicator, and a
 * highlight offset when an event is attached to the dialog.
 */
ig.module("game.feature.xeno-dialogs.gui.xeno-icon").requires("impact.feature.gui.gui", "impact.feature.gui.base.box", "impact.feature.gui.base.basic-gui", "game.feature.interact.map-interact", "game.feature.msg.gui.msg-skip-hud").defines(function() {
    sc.XenoDialogIcon = ig.BoxGui.extend({
        ...
        setText: function(text, xenoDialog) {
            this.xenoDialog = xenoDialog;
            this.textGui.setText(text || "This is an empty test");
            var paddingY = this.xenoDialog.currentEvent ? 3 : 2,
                width = Math.max(30, this.textGui.hook.size.x + (this.xenoDialog.currentEvent ? 6 : 4) * 2);
            this.setSize(width, this.textGui.hook.size.y + paddingY * 2);
            this.setPivot(this.hook.size.x / 2, this.hook.size.y);
        ...
        onSkipInteract: function(msg) {
            msg == sc.SKIP_INTERACT_MSG.SKIPPED && (this.textGui.textBlock.isFinished() ? this.xenoDialog._showNextMessage() : this.textGui.finish());
            this.updateSkipIcon()
        },
        setIconState: function() {},
        isActive: function(state) {
            return state != sc.INPUT_FORCER_ENTRIES.HIDDEN
        },
        remove: function() {
