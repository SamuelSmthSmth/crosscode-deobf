 ...
 * @description ig.GUI.GamepadBox: the HUD box announcing the gamepad is
 *   connected, with an info button explaining gamepad controls.
 */
ig.module("game.feature.gui.widget.gamepad-box").requires("impact.base.image", "impact.feature.gui.gui").defines(function() {
	ig.GUI.GamepadBox = ig.GuiElementBase.extend({
		...
		text: null,
		gamepadActive: false,
		init: function(focusManager, key, focusDir) {
			this.parent();
			this.setSize(180, 26);
			this.hook.zIndex = 90;
			this.infoButton = new sc.ButtonGui("?", 28);
			...
			this.infoButton.onButtonPress = function() {
				var text = ig.lang.get("sc.gui.gamepad-box.info-title") + "\n\n" + ig.lang.get("sc.gui.gamepad-box.info-text"),
					text = new sc.CenterMsgBoxGui(text, {
						maxWidth: 300,
						speed: ig.TextBlock.SPEED.IMMEDIATE
					}, "black", 0.9);
				text.hook.zIndex = 1500;
				ig.gui.addGuiElement(text)
			}.bind(this);
			focusManager.addFocusGui(this.infoButton, key, focusDir);
		...
		updateDrawables: function(drawables) {
			drawables.addGfx(this.gfx, 0, 0, 0, 160, this.hook.size.x, this.hook.size.y)
		}
	})
});
ig.baked = !0;
