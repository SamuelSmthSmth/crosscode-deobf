 ...
 * Source module: `assets/js/game.compiled.js` → `ig.module("game.feature.menu.gui.shop.shop-start")`.
 *
 * `sc.ShopStartMenu`: the buy/sell chooser popup shown when entering a
 * shop (sell hidden for buy-only shops), and `sc.ShopStartTitle`: the
 * floating "— shop name —" title bar above it.
 */
ig.module("game.feature.menu.gui.shop.shop-start")
    .requires("impact.feature.gui.gui", "impact.feature.gui.base.basic-gui", "game.feature.menu.gui.menu-misc", "game.feature.gui.base.button")
    .defines(function () {
    ...
    sc.ShopStartMenu = ig.BoxGui.extend({
        ...
        init: function () {
            ...
            var shop = ig.database.get("shops")[sc.menu.shopID];
            sc.MENU_SHOP_TYPES[shop.shopType] == sc.MENU_SHOP_TYPES.BUY && this.sell.setActive(false);
        ...
        hide: function (scale, immediate) {
            sc.menu.buttonInteract.removeButtonGroup(this.buttongroup);
            this.doStateTransition(scale ? "HIDDEN_SCALE" : "HIDDEN", immediate)
        },

        onButtonPress: function (button) {
            button.data == 0 ? sc.menu.setShopState(sc.MENU_SHOP_STATE.BUY) : button.data == 1 && sc.menu.setShopState(sc.MENU_SHOP_STATE.SELL)
        }
    });
    ...
    sc.ShopStartTitle = ig.BoxGui.extend({
        ...
        show: function () {
            var shop = ig.database.get("shops")[sc.menu.shopID],
                name = shop ? ig.LangLabel.getText(shop.name) : "Unknown Shop Name";
