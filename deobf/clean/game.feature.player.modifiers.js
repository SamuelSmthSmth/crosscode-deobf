 ...
 * Source module: `assets/js/game.compiled.js` → `ig.module("game.feature.player.modifiers")`.
 *
 * `sc.MODIFIERS`: the stat-modifier registry (icon index + display options
 * per modifier). After the table, each entry gets its `order` index assigned
 * (the order in which modifiers are displayed). Data kept value-identical.
 */
ig.module("game.feature.player.modifiers")
    ...
    var order = 0,
        key;
    for (key in sc.MODIFIERS) {
        sc.MODIFIERS[key].order = order;
        order++
    }
});
ig.baked = !0;
