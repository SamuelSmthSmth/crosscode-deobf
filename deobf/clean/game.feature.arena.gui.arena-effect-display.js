ig.module("game.feature.arena.gui.arena-effect-display").requires("impact.feature.gui.gui").defines(function() {
    sc.ArenaMedalEffect = ig.GuiElementBase.extend({
        ...
        show: function(medal, isTrophy) {
            this.medal = medal || false;
            this.isTrophy = isTrophy || false;
        ...
        hide: function(clearEffects) {
            clearEffects && this.display.clearEffects();
            this.display &&
                this.display.remove(true);
            this.doStateTransition("HIDDEN", true, true)
        },
        getMedalEffectName: function(medal, isTrophy) {
            if (isTrophy) switch (medal) {
                case 1:
        ...
        onLoaded: function(display) {
            if (display.npc) {
                display.setPos(this.container.hook.size.x / 2 - display.hook.size.x / 2, this.container.hook.size.y / 2 - display.hook.size.y /
                    2);
                display.playEffect("arena", this.getMedalEffectName(this.medal, this.isTrophy), {
                    callback: this,
        ...
        onEffectEvent: function(event) {
            if (event.state == ig.EFFECT_STATE.ENDED) this.effectDone = true
        }
    })
});
ig.baked = !0;
