ig.module("game.feature.arena.gui.arena-rush-gui").requires("impact.feature.gui.gui", "impact.feature.gui.base.basic-gui", "impact.feature.gui.base.box", "game.feature.gui.base.text", "game.feature.menu.gui.menu-misc", "game.feature.menu.gui.arena.arena-misc", "game.feature.arena.gui.arena-effect-display", "game.feature.gui.widget.modal-dialog", "game.feature.arena.gui.arena-trophy-gui").defines(function() {
    sc.ArenaRushOverview = ig.GuiElementBase.extend({
        ...
        init: function(callback) {
            ...
            callback = new ig.ColorGui("#7E7E7E", 280, 1);
            callback.setAlign(ig.GUI_ALIGN_X.CENTER, ig.GUI_ALIGN_Y.TOP);
            callback.setPos(0, 13);
            this.content.addChildGui(callback);
            ...
            callback = new ig.ColorGui("#7E7E7E", 280, 1);
            callback.setAlign(ig.GUI_ALIGN_X.CENTER, ig.GUI_ALIGN_Y.TOP);
            callback.setPos(0, 13 + this.list.hook.size.y - 1);
            this.content.addChildGui(callback);
            ...
            callback = ig.lang.get("sc.gui.arena.medals")[this.medalType - 1];
            this.medalFooter = new sc.TextGui("\\c[4]" + callback + "\\c[0]", {
                font: sc.fontsystem.smallFont,
                maxWidth: 76,
                textAlign: ig.Font.ALIGN.RIGHT,
                linePadding: -3
            });
            this.medalFooter.setAlign(ig.GUI_ALIGN.X_RIGHT, ig.GUI_ALIGN.Y_BOTTOM);
                ...
                if (this.timer <= 0)
                    if (this.currentIndex == -1) {
                        ...
                        var entry = this.entries[this.currentIndex],
                            medalEntry = new sc.ArenaCupOverview.MedalEntry(entry.id, entry.medal),
                            row = ~~(entry.id / 10);
                        medalEntry.setPos(1 + entry.id %
                            10 * 18, 2 + row * 18);
        ...
        addMedals: function() {
            ...
            this.list.recalculateScrollBars(true);
            var rushScores = sc.arena.runtime.rushScores;
            if (rushScores)
                for (var i = 0; i < rushScores.length; i++) {
                    var score = rushScores[i];
                    this.entries.push({
                        id: i,
                        medal: score.medal || 0,
                        time: score.time || 0,
        ...
        skip: function() {
            if (!this.done && this.addEntries) {
                ...
                for (var i = this.currentIndex; i < this.entries.length; i++) {
                    var entry = this.entries[this.currentIndex],
                        medalEntry = new sc.ArenaCupOverview.MedalEntry(entry.id, entry.medal, true),
                        row = ~~(entry.id / 10);
