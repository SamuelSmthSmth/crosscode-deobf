 ...
 * Source module: `assets/js/game.compiled.js` → `ig.module("game.feature.menu.gui.item.item-menu")`.
 *
 * `sc.ItemMenu`: the Items submenu container. Hosts the status panels
 * (equip/equip-modifiers/default/buffs/favorites/trade), the consumable buff
 * help hint, the tabbed item list and the three top-bar hotkeys (help,
 * set-favorite, sort). Routes SORT_LIST / ITEM_CHANGED_TAB menu events.
 */
ig.module("game.feature.menu.gui.item.item-menu")
    ...
    sc.ItemMenu = sc.BaseMenu.extend({
        ...
        init: function () {
            ...
            this.statusBuffBox.annotation = {
                size: {
                    x: this.statusBuffBox.hook.size.x + 2,
        ...
        createHelpGui: function () {
            if (!this.helpGui) {
                this.helpGui = new sc.HelpScreen(this, ig.lang.get("sc.gui.menu.help-texts.inventory.title"), ig.lang.get("sc.gui.menu.help-texts.inventory.pages"), function () {
        ...
        _updateSortMenuButton: function (sortText) {
            var prefix = "\\i[help3]" + ig.lang.get("sc.gui.menu.item.sort-title") + ": ";
            this.hotkeySort.setText(prefix + ("\\c[3]" + sortText + "\\c[0]"));
        ...
        modelChanged: function (menu, event, data) {
            if (menu == sc.menu) {
                if (event == sc.MENU_EVENT.SORT_LIST) {
                    this._updateSortMenuButton(data.text)
                } else if (event == sc.MENU_EVENT.ITEM_CHANGED_TAB) {
                    this.sortMenu.active && this.sortMenu.hideSortMenu();
                    if (data != sc.menu.itemCurrentTab) {
                        this._updateSortMenuButton(this.listBox.getCurrentSortText());
                        sc.menu.itemCurrentTab == 0 ? this.hotkeySort.setActive(false) : this.hotkeySort.setActive(true)
                    }
                }
