ig.module("game.feature.menu.gui.quests.quest-details").requires("impact.feature.gui.base.box", "impact.feature.gui.gui", "impact.feature.interact.button-interact", "game.feature.interact.button-group", "game.feature.menu.gui.quests.quest-entries", "game.feature.menu.gui.quests.quest-misc", "game.feature.npc.gui.npc-display-gui").defines(function() {
    ...
    sc.QuestDetailTasks = ig.BoxGui.extend({
        ...
        scroll: function(a, b) {
            this.container.scrollY(a, b, 0.05)
        },
        show: function(a, b, c) {
            var e = sc.quests.getCurrentTask(a, true) + 1,
                f = null,
                g = this.content,
                h = 5;
            if (!(g.hook.children.length >= 1 && c)) {
                g.removeAllChildren();
                for (c = 0; c < e; c++) {
                    f = new sc.TaskEntry(c, a, false, true);
                    f.doStateTransition("DEFAULT", true);
                    f.setPos(4, h);
                    h = h + (f.hook.size.y + 5);
                    ig.langEdit &&
                        ig.langEdit.submitCustomFile("Quest Task [" + (c + 1) + "]: " + a.name, a.tasks[c].task, "data/database.json");
                    g.addChildGui(f)
                }
                g.setSize(266, h);
    ...
    sc.QuestDetailsSolved = ig.GuiElementBase.extend({
        ...
        show: function(a) {
            this.endDescription.setText(a.endDescription);
    ...
    sc.QuestCharacterView = ig.BoxGui.extend({
        ...
        setCharacter: function(a, b) {
            if (a) {
                ...
                if (this.display) {
                    b ? this.display.remove(true) :
                        this.display.doStateTransition("HIDDEN", false, true);
                    this.display = null
                }
                this.doStateTransition("HIDDEN", true)
            }
        ...
        centerNPC: function(a) {
            if (a.npc) {
                this.hideBackground = a.hideBackground;
                a.setPos(this.container.hook.size.x / 2 - a.hook.size.x / 2 - 1 + a.displayOffset.x, this.container.hook.size.y / 2 - a.hook.size.y / 2 + a.displayOffset.y)
            }
        },
        updateDrawables: function(a) {
            this.hideBackground || this.parent(a)
        }
    });
    sc.QuestDetailsView = sc.QuestBaseBox.extend({
        ...
        init: function(a) {
            ...
            this.expGui.setPos(20, a);
            this.addChildGui(this.expGui);
            a = a + 16;
            this.moneyGui = new sc.TextGui("");
            this.moneyGui.setPos(20, a);
            this.addChildGui(this.moneyGui);
            a = a + 16;
            this.cpGui = new sc.TextGui("");
            this.cpGui.setPos(20, a);
            this.addChildGui(this.cpGui);
            a = a + 16;
            this.itemsGui = new ig.GuiElementBase;
            this.itemsGui.setPos(20, a);
            this.addChildGui(this.itemsGui);
            a = new ig.ImageGui(this.gfx, 418, 33, 12, 18);
            a.setPos(7, 25);
            this.addChildGui(a);
            a = new ig.ColorGui("#545454", 155, 1);
            a.setPos(20, 39);
            this.addChildGui(a);
            this.lines.push(a);
            a = new ig.ImageGui(this.gfx, 440, 64, 8, 8);
            a.setPos(21, 62);
            this.addChildGui(a);
            a = new ig.ImageGui(this.gfx, 433, 34, 22, 16);
            a.setPos(8, 85);
            this.addChildGui(a);
            a = new ig.ColorGui("#545454", 156, 1);
            a.setPos(19, 97);
            this.addChildGui(a);
            this.lines.push(a);
            a = new ig.ImageGui(this.gfx, 457, 37, 14, 10);
            a.setPos(8, 148);
            this.addChildGui(a);
            a = new ig.ColorGui("#545454", 152, 1);
            a.setPos(23, 154);
            this.addChildGui(a);
            this.lines.push(a);
            this.personCharGui =
                new sc.QuestCharacterView;
        ...
        checkTaskSwitch: function() {
            ...
            (this.taskSwitch = !this.taskSwitch) ? this.activeView.show(this.currentQuest, false,
                true): this.activeView.hide()
        },
        ...
        _setQuest: function(a) {
            this.currentQuest = a;
            this.titleGui.setText(a.name);
            this.descriptionGui.setText(a.description);
            this.locationArea.setText(a.location.area);
            this.locationMap.setText(a.location.map);
            this.personCharGui.setCharacter(a.character);
            this.setLevel(a.level);
            var d = sc.quests.isQuestSolved(a.id),
                c = 160,
                e = null;
            this.setElite(a.elite, d);
            var f = a.hideRewards & !d;
            this.personTextGui.setText(d ? a.personAfter || a.person || "???" : a.person || "???");
            ...
            if (d) {
                ...
                for (e = this.lines.length; e--;) this.lines[e].color =
                    "#008277";
                this.taskButton.setActive(true)
            } else {
                if (a.elite) {
                ...
                for (e = this.lines.length; e--;) this.lines[e].color = a.elite ? "#8d0000" : "#545454";
            ...
            if (f) {
                ...
                if (a.rewards.exp) {
                    e = "\\i[exp]" + (sc.model.player.getRawExpGain(a.rewards.exp.exp, a.level, sc.LEVEL_CURVES.QUEST) + "* ");
                    e = e + (a.rewards.exp.bonus ? "+" + a.rewards.exp.bonus : "");
                    this.expGui.setText(e);
                    this.expGui.hook.pos.y = c;
                    c = c + 16;
                ...
                if (a.rewards.money) {
                    this.moneyGui.setText("\\i[credit]" + (a.rewards.money || 0));
                    this.moneyGui.hook.pos.y = c;
                    c = c + 16
                } else this.moneyGui.setText("");
                if (a.rewards.cp) {
                    a.rewards.cp.amount == 1 ? this.cpGui.setText("\\i[cp]" + b(a.rewards.cp.element)) : this.cpGui.setText("\\i[cp]" + b(a.rewards.cp.element) + "x " + (a.rewards.cp.amount || 0));
                    this.cpGui.hook.pos.y = c;
                    c = c + 16
                } else this.cpGui.setText("")
            }
            this.itemsGui.removeAllChildren();
            if (a.rewards.items) {
                this.itemsGui.hook.pos.y =
                    c;
                for (var c = this.itemsGui, e = a.rewards.items, g = this.gfx, h = 0, i = 0; i < e.length; i++) {
                    var j = e[i].amount,
                        k = sc.inventory.getItem(e[i].id),
                        l = "\\i[" + (k.icon + sc.inventory.getRaritySuffix(k.rarity || 0) || "item-default") + "]",
                        l = f ? l + "?????????????" : l + ig.LangLabel.getText(k.name);
                    j > 1 && (l = l + (" x " + j));
                    j = 0;
                    ...
                    l.level = j;
                    l.numberGfx = g;
                    j > 0 && !f && l.setDrawCallback(function(a, b) {
            ...
            if (d) {
                ...
                this.solvedView.show(a)
            } else {
                this.solvedView.hide();
                this.activeView.show(a)
            }
        },
        ...
        modelChanged: function(a, b, c) {
            a == sc.menu && b == sc.MENU_EVENT.QUEST_ENTER_DEAILS && this.show(c)
        }
    })
});
ig.baked = !0;
