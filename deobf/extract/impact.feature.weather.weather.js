ig.module("impact.feature.weather.weather").requires("impact.base.game", "impact.feature.weather.clouds", "impact.feature.weather.fog", "impact.feature.weather.rain").defines(function() {
    ...
    ig.WeatherInstance = ig.Cacheable.extend({
        ...
        init: function(b) {
            ...
            if (b = this.config.particles)
                for (var a = 0; a < b.length; ++a) this.particleSpawners.push(new ig.EnvParticleSpawner(b[a].type))
        },
        getCacheKey: function(b) {
            return b
        },
        onCacheCleared: function() {
            for (var b = this.particleSpawners.length; b--;) this.particleSpawners[b].decreaseRef();
    ...
    ig.Weather = ig.GameAddon.extend({
        ...
        setWeather: function(b, a) {
            this.currentWeather = b;
            this.updateWeather(a)
        },
        addExtraParticles: function(b, a) {
            this.removeExtraParticles(b, true);
            this.extraParticles.push({
                spawner: b,
                count: a
            });
            ig.envParticles.addSpawner(b,
                a)
        },
        removeExtraParticles: function(b, a) {
            for (var d = this.extraParticles.length; d--;) this.extraParticles[d].spawner == b && this.extraParticles.splice(d, 1);
            a || ig.envParticles.addSpawner(b, 0)
        },
        ...
        onDeferredUpdate: function() {
            if (!ig.game.paused) {
                if (this.lightMapDarkness.timer) {
                    ...
                    var b = this.lightMapDarkness.timer / 2,
                        b = b * this.lightMapDarkness.last + (1 - b) * this.lightMapDarkness.target;
                    ig.light.lightMapDarkness = b
                }
                if (this.lightMapBrightness.timer) {
                    this.lightMapBrightness.timer =
                        this.lightMapBrightness.timer - ig.system.tick;
                    if (this.lightMapBrightness.timer < 0) this.lightMapBrightness.timer = 0;
                    b = this.lightMapBrightness.timer / 2;
                    b = b * this.lightMapBrightness.last + (1 - b) * this.lightMapBrightness.target;
                    ig.light.lightMapBrightness = b
                }
                this.clouds.update();
                this.fog.update();
                this.rain.update()
        ...
        onMidDraw: function() {
            ...
            if (ig.perf.weather && this.subOverlay) {
                ...
                b.fillStyle = this.subOverlay.brightColor;
                b.fillRect(0,
                    0, ig.system.realWidth, ig.system.realHeight);
                b.globalCompositeOperation = "source-over";
                b.fillStyle = this.subOverlay.darkColor;
        ...
        updateWeather: function(b) {
            var a = this.currentWeather.config;
            if (a.blackCorners) {
                var d = a.blackCorners;
                ig.overlay.setCorner("BLACK", d.alpha, d.time, d.blinkAlpha)
            } else ig.overlay.setCorner("BLACK", 0, b ? 0 : 2);
            if (a.whiteCorners) {
                d = a.whiteCorners;
                ig.overlay.setCorner("WHITE", d.alpha, d.time,
                    d.blinkAlpha)
            } else ig.overlay.setCorner("WHITE", 0, b ? 0 : 2);
            if (a.clouds) {
                d = a.clouds;
                this.clouds.setClouds(d.density, d.vel, d.alpha, b)
            } else this.clouds.clearClouds(b);
            if (a.fog) {
                d = a.fog;
                this.fog.setFog(d.alpha, d.vel, d.zoom, b)
            } else this.fog.clearFog(b);
            a.rain ? this.rain.setRain(a.rain, b) : this.rain.clearRain(b);
            a.darkness ? this.darknessHandle.setIntensity(a.darkness, b ? 0 : 2) : this.darknessHandle.setIntensity(0, b ? 0 : 2);
            this.outside = a.outside;
            var d = a.lightMapDarkness,
                c = a.lightMapBrightness;
            d == void 0 && (d = 0.6);
            c ==
                void 0 && (c = 1);
            if (b) {
                ig.light.lightMapDarkness = d;
                ig.light.lightMapBrightness = c
            } else {
                this.lightMapDarkness.last = ig.light.lightMapDarkness;
                this.lightMapDarkness.target = d;
                this.lightMapDarkness.timer = 2;
                this.lightMapBrightness.last = ig.light.lightMapBrightness;
                this.lightMapBrightness.target = c;
                this.lightMapBrightness.timer = 2
            }
            this.subOverlay = a.subOverlay || null;
            this.restoreParticles(b)
        },
        restoreParticles: function(b) {
            ig.envParticles.clear(b);
            for (var a = this.currentWeather.config, d = this.currentWeather.particleSpawners,
                    c = d.length; c--;) ig.envParticles.addSpawner(d[c], a.particles[c].quantity, b);
            for (c = this.extraParticles.length; c--;) ig.envParticles.addSpawner(this.extraParticles[c].spawner, this.extraParticles[c].count, b)
        }
    });
