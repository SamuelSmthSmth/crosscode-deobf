ig.module("game.feature.gui.screen.pause-screen").requires("impact.feature.gui.gui", "impact.base.image", "game.feature.gui.base.button", "game.feature.menu.gui.save.save-menu", "game.feature.gui.widget.modal-dialog").defines(function() {
    sc.PauseScreenGui = ig.GuiElementBase.extend({
        ...
        init: function() {
            ...
            this.toTitleButton = new sc.ButtonGui(ig.lang.get("sc.gui.pause-screen.to-title"),
            ...
            this.toTitleButton.onButtonPress = function() {
                ...
                sc.Dialogs.showYesNoDialog(ig.lang.get("sc.gui.dialogs.titleConfirm"), sc.DIALOG_INFO_ICON.QUESTION, function(a) {
            ...
            this.cancelButton.onButtonPress = function() {
                ig.canLeavePauseMenu = false;
                sc.Dialogs.showYesNoDialog(ig.lang.get("sc.gui.dialogs.cancelConfirm"), sc.DIALOG_INFO_ICON.QUESTION, function(a) {
                    ig.canLeavePauseMenu =
                        true;
                    if (a.data == 0) {
                        ig.interact.removeEntry(this.buttonInteract);
                        sc.model.enterRunning();
            ...
            this.saveGameButton = new sc.ButtonGui(ig.lang.get("sc.gui.pause-screen.save-game"),
                sc.BUTTON_DEFAULT_WIDTH);
            ...
            this.arenaRestart.onButtonPress = function() {
                ig.canLeavePauseMenu = false;
                sc.Dialogs.showYesNoDialog(ig.lang.get("sc.gui.arena.dialogs." + (sc.arena.runtime.rush ? "restart" : "repeat")), sc.DIALOG_INFO_ICON.QUESTION, function(a) {
                    ig.canLeavePauseMenu = true;
                    if (a.data == 0) {
            ...
            this.arenaLobby.onButtonPress = function() {
                ig.canLeavePauseMenu = false;
                sc.Dialogs.showYesNoDialog(ig.lang.get("sc.gui.arena.dialogs.lobby"), sc.DIALOG_INFO_ICON.QUESTION,
                    function(a) {
                        ig.canLeavePauseMenu = true;
                        if (a.data == 0) {
            ...
            var b = ig.lang.get("sc.gui.pause-screen.lang-fix");
            this.infoGui = new sc.TextGui(b, {
        ...
        updateDrawables: function(b) {
            b.addPattern(this.constructor.PATTERN, 0, 0, 0, 0, this.hook.size.x, this.hook.size.y)
        },
        modelChanged: function(b, a) {
            if (b == sc.model) {
                if (a ==
                    sc.GAME_MODEL_MSG.SUB_STATE_CHANGED && !sc.model.isReset() && sc.model.currentSubState != sc.GAME_MODEL_SUBSTATE.MENU) {
                    var d = sc.menu.directMode ? true : sc.model.isPaused();
            ...
            } else if (b == sc.menu && a == sc.MENU_EVENT.FULL_MENU_ENTER) this.hook.invisibleUpdate = false
        },
        updateButtons: function(b) {
            ...
            var a = this.buttonGroup.current.y,
            ...
            if (sc.arena.active && c && c.roundStarted && !c.roundFinished && !c.roundEndPre && sc.model.currentState != sc.GAME_MODEL_STATE.CUTSCENE) {
                ...
                this.buttonGroup.addFocusGui(this.arenaLobby,
                    0, 3);
                this.arenaLobby.setPos(3, 3);
                c = this.arenaLobby.hook.size.y;
                this.resumeButton.setPos(3, c * 3 + 15);
                this.optionsButton.setPos(3, c * 2 + 11);
                this.arenaRestart.setPos(3, 3 + c + 4)
            } else {
                this.addChildGui(this.toTitleButton);
                this.addChildGui(this.resumeButton);
                this.addChildGui(this.saveGameButton);
                ...
                if (sc.model.currentState == sc.GAME_MODEL_STATE.CUTSCENE && !sc.model.skipBlock) {
                    ...
                    this.cancelButton.setText("\\c[1]" + sc.model.cancelButtonText.toString() +
                        "\\c[0]");
    ...
    ig.GUI.PauseScreenLabel = ig.GuiElementBase.extend({
        ...
        update: function() {
            ...
            var b = this.timer / this.timerTime;
