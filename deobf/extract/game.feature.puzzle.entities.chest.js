ig.module("game.feature.puzzle.entities.chest").requires("impact.base.actor-entity", "impact.base.entity", "game.feature.interact.map-interact", "game.feature.inventory.inventory", "game.feature.menu.menu-model").defines(function() {
    sc.CHEST_TYPE = {};
    sc.CHEST_KEY = {};
    ...
    ig.ENTITY.Chest = ig.AnimatedEntity.extend({
        ...
        init: function(b, a, d, c) {
            ...
            this.interactEntry.blockedDuringCombat =
                true;
            this._trigger = c.trigger || null;
            this._noTrack = c.noTrack || false;
            b = sc.CHEST_TYPE[c.chestType] || sc.CHEST_TYPE["default"];
            this.item = c.item || 0;
            this.chestType = b;
            this.coll.setSize(17, 13, 12);
            this.chestVariable = (b = c.variable || "") && b != "" ? b : "map.chest_" + this.mapId;
            this._itemName = ig.LangLabel.getText(sc.inventory.getItem(this.item).name) || "NULL";
            this.amount = c.amount || 1;
            this.initAnimations({
                SUB: [{
                        shapeType: "Y_FLAT",
                        sheet: {
            ...
            if (c.hideCondition) this.hideManager =
                new ig.EntityHideManager(c.hideCondition);
            if (c.detectCondition) this.detectCondition = new ig.VarCondition(c.detectCondition)
        },
        _initGfx: function() {
            this._initialized = true;
            var b = sc.map.getAreaItemType(this.item);
            if (b == "DUNGEON_KEY") this.chestType = sc.CHEST_TYPE.Key;
            else if (b == "DUNGEON_MASTER_KEY") this.chestType = sc.CHEST_TYPE.MasterKey;
        ...
        onEffectEvent: function(b) {
            ...
            } else b.isDone() && this._reallyOpenUp()
        },
        show: function(b) {
            this.parent(b);
            if (!b) {
        ...
        _updateIcon: function() {
            var b = this.chestType.item ? false : true;
            b || (b = sc.model.player.hasItem(this.chestType.item));
            b ? this.interactEntry.setIcon(this.mapIcons) : this.interactEntry.setIcon(this.mapIconsKey)
        },
        ...
        onInteraction: function() {
            ...
            if (b.currentAction && b.currentAction.eventAction) return false;
            ...
            var a = [{
                ...
                d = this.chestType.openEffect ? 1.2 : 0.8;
            if (this._trigger && !ig.vars.get(this._trigger)) {
                a.push({
        ...
        _reallyOpenUp: function() {
            if (!this.isOpen) {
                ...
                sc.ItemDropEntity.spawnDrops(this,
                    ig.ENTITY_ALIGN.CENTER, ig.game.playerEntity, this.item, this.amount, sc.ITEM_DROP_TYPE.CHEST);
                if (!this._noTrack) {
        ...
        animationEnded: function(b) {
            b == "open" && this.setCurrentAnim("end", true, null, true)
        },
