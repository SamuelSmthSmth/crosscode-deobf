ig.module("game.feature.menu.gui.equip.equip-menu").requires("impact.feature.gui.gui", "game.feature.gui.plug-in", "game.feature.menu.gui.base-menu", "game.feature.menu.gui.main-menu", "game.feature.menu.gui.equip.equip-status", "game.feature.menu.gui.equip.equip-bodypart", "game.feature.inventory.inventory", "game.feature.menu.gui.help.help-menu").defines(function() {
    sc.EquipMenu = sc.BaseMenu.extend({
        ...
        init: function() {
            ...
            this.quickSelectButtonGroup.addSelectionCallback(function(a) {
            ...
            this.globalButtons.rightArm.setPos(sc.options.hdMode ? 221 :
                178, 119);
            ...
            this.initEquipIcon(this.globalButtons.torso,
                sc.model.player.equip.torso);
            ...
            this.quickSelectButtonGroup.addFocusGui(this.globalButtons.leftArm.button,
                2, 1);
            ...
            for (var b in this.globalButtons) this.globalButtons[b] && this.addChildGui(this.globalButtons[b]);
            ...
            this.hotkeys.switch.onButtonPress = function() {
                var a = ig.input.mouseGuiActive ? sc.menu.buttonInteract.mouseOverGui : this.rightContainer.itemList.list.buttonGroup.getCurrentElement();
                this.statusContainer.toggleViewMode(a);
                ig.interact.setBlockDelay(0.2)
            }.bind(this)
        },
        initEquipIcon: function(b, a) {
            var d = sc.inventory.getItemRarity(a),
                c = sc.inventory.getItemSubType(a),
                e = sc.inventory.getItemLevel(a);
            b.setEquip(d, c, e, true)
        },
        ...
        createHelpGui: function() {
            if (!this.helpGui) {
                this.helpGui =
                    new sc.HelpScreen(this, ig.lang.get("sc.gui.menu.help-texts.equip.title"), ig.lang.get("sc.gui.menu.help-texts.equip.pages"), function() {
        ...
        onStatusButtonCheck: function() {
            return ig.interact.isBlocked() ?
                false : sc.control.menuHotkeyHelp2()
        },
        onStatusButtonPress: function() {
            if (sc.menu.previousMenu == sc.MENU_SUBMENU.START) {
                sc.menu.currentBodyPart != 0 && sc.menu.popBackCallback();
        ...
        _exitMenu: function() {
            var b = sc.menu.previousMenu;
            sc.menu.popBackCallback();
            sc.menu.popMenu();
            b == sc.MENU_SUBMENU.STATUS ? sc.menu.previousMenu = sc.MENU_SUBMENU.START : sc.menu.exitEquipMenu()
        },
        _moveBodyPartLines: function(b, a) {
            switch (b) {
                case sc.MENU_EQUIP_BODYPART.NONE:
                    for (var d in this.globalButtons) this.globalButtons[d] && this.globalButtons[d].resetLine(a);
                    break;
                case sc.MENU_EQUIP_BODYPART.HEAD:
                    this._pullInAllButtons(this.globalButtons.head,
                        a);
                    this.globalButtons.head.showLine(true);
                    break;
                case sc.MENU_EQUIP_BODYPART.RIGHT_ARM:
                    this._pullInAllButtons(this.globalButtons.rightArm, a);
                    this.globalButtons.rightArm.showLine(true);
                    break;
                case sc.MENU_EQUIP_BODYPART.LEFT_ARM:
                    this._pullInAllButtons(this.globalButtons.leftArm, a);
                    this.globalButtons.leftArm.showLine(true);
                    break;
                case sc.MENU_EQUIP_BODYPART.TORSO:
                    this._pullInAllButtons(this.globalButtons.torso, a);
                    this.globalButtons.torso.showLine(true);
                    break;
                case sc.MENU_EQUIP_BODYPART.FEET:
                    this._pullInAllButtons(this.globalButtons.feet,
                        a);
                    this.globalButtons.feet.showLine(true)
            }
        },
        _pullInAllButtons: function(b, a) {
            var d = null,
                c;
            for (c in this.globalButtons)(d = this.globalButtons[c]) && d != b && d.hideLine(a)
        },
        _updateSortHotkey: function(b) {
            var a = "\\i[help3]" + ig.lang.get("sc.gui.menu.item.sort-title") + ": ";
            this.hotkeys.sort.setText(a + ("\\c[3]" + b + "\\c[0]"));
            sc.menu.updateHotkeys()
        },
        _updateMouseButtons: function(b, a) {
            var d = b.equipID ? sc.inventory.getItemRarity(b.equipID) : null,
                c = b.equipID ? sc.inventory.getItemSubType(b.equipID) : null,
                e = b.equipID ? sc.inventory.getItemLevel(b.equipID) :
                null;
            switch (sc.menu.currentBodyPart) {
                case sc.MENU_EQUIP_BODYPART.NONE:
                    this.globalButtons.head.setEquip(d, c, e, a);
                    this.globalButtons.rightArm.setEquip(d, c, e, a);
                    this.globalButtons.leftArm.setEquip(d, c, e, a);
                    this.globalButtons.torso.setEquip(d, c, e, a);
                    this.globalButtons.feet.setEquip(d, c, e, a);
                    break;
                case sc.MENU_EQUIP_BODYPART.HEAD:
                    this.globalButtons.head.setEquip(d, c, e, a);
                    break;
                case sc.MENU_EQUIP_BODYPART.RIGHT_ARM:
                    ...
                    break;
                case sc.MENU_EQUIP_BODYPART.LEFT_ARM:
                    this.globalButtons.leftArm.setEquip(d,
                        c, e, a);
                    break;
                case sc.MENU_EQUIP_BODYPART.TORSO:
                    this.globalButtons.torso.setEquip(d, c, e, a);
            ...
            if (b == sc.menu)
                if (a == sc.MENU_EVENT.SELECTED_BODYPART) {
                    if (b.currentBodyPart != b.previousBodyPart) {
                        if (b.currentBodyPart == sc.MENU_EQUIP_BODYPART.NONE) this.hotkeys.sort.setActive(false);
                        else {
                            this._updateSortHotkey(this.rightContainer.getCurrentSortText());
                            this.hotkeys.sort.setActive(true)
                        }
                        this._moveBodyPartLines(b.currentBodyPart)
                    }
                } else a ==
                    sc.MENU_EVENT.SORT_LIST && this._updateSortHotkey(d.text);
            else if (b == sc.model.player && a == sc.PLAYER_MSG.EQUIP_CHANGE) {
                this.overview.updateNumbers();
                this._updateMouseButtons(d, false)
            }
        },
        showMenu: function(b, a) {
            this.addObservers();
            sc.menu.menuHost == 0 && sc.menu.setHost(sc.MENU_SUBMENU.EQUIPMENT);
            this.statusContainer.showMenu();
            this.overview.show();
            if (sc.menu.menuHost == sc.MENU_SUBMENU.EQUIPMENT)
                if (a == sc.MENU_SUBMENU.STATUS) {
                    sc.menu.statusPage != sc.MENU_STATUS_PAGES.MAIN && sc.menu.moveLeaSprite(0, -101, sc.MENU_LEA_STATE.SMALL,
                        true);
            ...
            for (var d in this.globalButtons) this.globalButtons[d] && this.globalButtons[d].showButton();
            sc.menu.currentBodyPart != 0 && a == sc.MENU_SUBMENU.STATUS && this._moveBodyPartLines(sc.menu.currentBodyPart, true);
            ig.interact.setBlockDelay(0.2);
            ...
            sc.menu.buttonInteract.addGlobalButton(this.hotkeys.status,
                this.onStatusButtonCheck.bind(this));
        ...
        hideMenu: function(b, a) {
            a != sc.MENU_SUBMENU.STATUS && sc.menu.moveLeaSprite(0,
                0, sc.MENU_LEA_STATE.LARGE);
            this.exitMenu(a)
        },
        exitMenu: function(b) {
            ...
            this.overview.hide();
            if (b == sc.MENU_SUBMENU.STATUS) this.rightContainer.tempHideMenu();
            else {
                this.rightContainer.hideMenu();
                sc.menu.menuHost = 0
            }
            this.helpGui && ig.gui.removeGuiElement(this.helpGui);
            this.helpGui = null;
            ...
            this.sortMenu.active && this.sortMenu.hideSortMenu();
            for (var a in this.globalButtons) this.globalButtons[a] && this.globalButtons[a].hideButton()
        }
    })
});
ig.baked = !0;
