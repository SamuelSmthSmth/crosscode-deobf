ig.module("game.feature.gui.hud.drop-hud").requires("game.feature.gui.hud.right-hud").defines(function() {
    sc.DropUpdateEntry = ig.GuiElementBase.extend({
        ...
        init: function(b) {
            this.parent();
            this.timer = 5;
            b = "\\c[3]" + sc.menu.getDropName(b) + "\\c[0] ";
            b = b + ig.lang.get("sc.gui.drop-hud.completed");
    ...
    sc.DropUpdateHud = sc.RightHudBoxGui.extend({
        ...
        _isInEntries: function(b) {
            for (var a = this.contentEntries.length; a--;)
                if (this.contentEntries[a].subGui.id ==
                    b) return this.contentEntries[a].subGui;
            ...
            if (!sc.model.isPaused() && !sc.model.isMenu() && !this.hidden) {
                for (var b = this.contentEntries.length, a = null; b--;) {
                    ...
                    if (a.timer <= 0) {
                        ...
                        if (b == 0 && this.contentEntries.length ==
                            0) a.hook.pivot.y = a.hook.size.y / 2;
                        else {
                            a.hook.pivot.y = 0;
        ...
        modelChanged: function(b, a, d) {
            if (b == sc.model)
                if (b.isReset()) {
                    this.clearContent();
                    this.hide()
                } else b.isCutscene() || b.isHUDBlocked() || sc.quests.hasQuestSolvedDialogs() ? this.hide() : !b.isCutscene() && (!b.isHUDBlocked() && this.contentEntries.length > 0 && !sc.quests.hasQuestSolvedDialogs()) && this.show();
            else b == sc.menu && (sc.model.player.getCore("MENU_BOTANICS") ?
                a == sc.MENU_EVENT.DROP_COMPLETED && sc.options.get("update-drop-style") == sc.UPDATE_DROP_STYLE.SMALL && d && this.addEntry(d) : this.hide())
        }
    })
});
ig.baked = !0;
