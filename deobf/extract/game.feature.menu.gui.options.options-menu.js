ig.module("game.feature.menu.gui.options.options-menu").requires("impact.feature.gui.gui", "game.feature.menu.gui.options.options-list").defines(function() {
    sc.OptionsMenu = sc.BaseMenu.extend({
        ...
        init: function() {
            ...
            this.hotkeyHelp.keepMouseFocus =
                true;
        ...
        onDefaultButtonPressed: function() {
            sc.Dialogs.showYesNoDialog(ig.lang.get("sc.gui.dialogs.resetAsk"), sc.DIALOG_INFO_ICON.WARNING, function(b) {
                b.data == 0 && sc.options.resetDefaultValues()
            }.bind(this))
        },
        ...
        createHelpGui: function() {
            if (!this.helpGui) {
                this.helpGui =
                    new sc.HelpScreen(this, ig.lang.get("sc.gui.menu.help-texts.options.title"), ig.lang.get("sc.gui.menu.help-texts.options.pages"), function() {
        ...
        modelChanged: function(b, a, d) {
            if (b == sc.menu)
                if (a == sc.MENU_EVENT.OPTION_LANG_POP_UP) {
                    this.langGui.show(d, this.onLangPopUpClose.bind(this));
                    this.blackBox.doStateTransition("DEFAULT")
                } else if (a == sc.MENU_EVENT.OPTION_CHANGED_TAB) {
                this.langGui.hide();
                this.blackBox.doStateTransition("HIDDEN")
            }
        }
